  ...
  return (
    ...
    >
      ...
      <div className="hidden w-35 items-center sm:flex">
        {printed ? (
          <span className="inline-flex h-6 items-center gap-1.25 rounded-full bg-brand-tint pl-2.25 pr-2.75 font-display text-xs font-bold tracking-[.02em] text-brand-deep">
            <CheckIcon size={12} className="text-brand-deep" strokeWidth={3} />
            {attendee.printCount > 1 ? `Printed ×${attendee.printCount}` : 'Printed'}
          </span>
        ) : (
          <span className="inline-flex h-6 items-center gap-1.5 rounded-full border border-amber px-3 font-display text-xs font-bold tracking-[.02em] text-amber-ink">
      ...
      <div className="flex w-23 flex-col items-end gap-0.75 sm:w-31">
        {printed ? (
          <Button
            ...
            disabled={!template}
            className="h-8.5 rounded-[9px] px-3.5 text-[13px] sm:px-4"
          >
            Reprint
          </Button>
        ) : (
          <Button
            onClick={() => onPreview(attendee)}
            disabled={!template}
            className="h-9.5 gap-1.75 rounded-[9px] px-3.5 text-sm tracking-[.01em] shadow-[0_6px_16px_rgba(111,162,63,.26)] sm:px-5"
          >
            <PrinterIcon size={14} />
            Print
          </Button>
