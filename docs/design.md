...
A **calm, fast kiosk tool** operated by staff standing at a live event. Every decision serves: *find a person → confirm status at a glance → print → move on*, in seconds, under pressure. Branded and crafted, never decorative at the cost of speed.
...
**Connect printer** (only when not connected): solid `green-500` button, white text, hover `green-600`. This is the *only* place an action lives in the dark frame.
